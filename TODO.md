...
- [ ] **补齐逻辑层覆盖率** — 基线：逻辑层合计 ≈82.6%，全目标 43.24%（约 80% 计数行是视图代码，见 `BLOCKERS.md` 说明 85% 总量目标为何不成立）。已完成 `BackupImporter`（69.6%→78.3%）与 `CSVTransactionService`（68.2%→88.2%）。逻辑层合计（仅单元测试）79.9%。下一批候选按未覆盖行排序：`Services/FinanceServices` 81.0%（3486 行，缺口最大）、`Core` 75.2%、`Services/BudgetServices` 75.4%、`Services/SystemServices` 77.1%。影响范围：测试。
