...
- [ ] **补齐逻辑层覆盖率** — 基线：逻辑层合计 ≈82.6%，全目标 43.24%（约 80% 计数行是视图代码，见 `BLOCKERS.md` 说明 85% 总量目标为何不成立）。已完成 `BackupImporter`（69.6%→78.3%）、`CSVTransactionService`（68.2%→88.2%）、`CategoryManagementService`（61.2%→85.8%）。逻辑层合计（仅单元测试）80.8%。下一批候选按未覆盖行排序：`LedgerQueryService` 67.6%、`CashFlowForecastService` 68.3%、`ReportBudgetSnapshotService` 56.4%、`ReminderNotificationService` 5.6%（后者大半是系统通知中心调用，可测部分有限）。影响范围：测试。
