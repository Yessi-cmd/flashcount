...
- [ ] **本轮 UX 改动缺 ADR** — `docs/decisions/` 按规定应记录重大决策。至少两项够格：分类格子交互模型（单点即选 + 明确的换小类入口，且两种失败方案要留下"别再试"的记录）、隐私锁放宽（去掉确认弹窗与切 tab 重锁）。影响范围：文档。
...
- [ ] **补齐覆盖率不足的模块** — 待覆盖率数据落地后按模块排序细化。目标 ≥85%。影响范围：测试。
