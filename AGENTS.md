...
**Persistence:** SwiftData with a versioned schema — `Schema(versionedSchema: FlashCountSchemaV4.self)` plus `FlashCountMigrationPlan` (see `FlashCount/Core/FlashCountSchema.swift`). SchemaV4 has 14 models: Transaction, Category, Ledger, RecurringRule, RecurringOccurrence, Budget, PhysicalAsset, CashPoolItem, CashPoolState, SavingsGoal, InstallmentBill, TransactionTemplate, Reminder, Subscription. The `Asset` account model was removed in July 2026 — the class survives only so SchemaV1/V2 and the V2→V3 conversion stage compile; never use it in new code. Reminders were migrated from a legacy JSON file into SwiftData; `FileReminderStore` (in `Services/DataServices/ReminderStore.swift`) is a read-only legacy codec kept for one-time import, alongside the live `ReminderDataService`.
...
| `SubscriptionService` | Aggregates subscription totals, persists add/edit/archive, and advances renewals (writing an expense by default, like installments) |
...
| `RoutineSpendingProfile` → `CashFlowForecastService` | Builds local P20/P50/P80 routine-spending rhythms from completed recorded weeks, then combines them with exact recurring/installment events into a balance interval |
...
- Subscriptions are **remind-first tracked obligations** (`Subscription`): they never auto-generate transactions. Confirming a renewal via `SubscriptionService.advanceRenewal` advances the date and writes an expense by default; the opt-out is only for hand-recorded payments (same invariant as installments). Do not route subscriptions through `RecurringRule` — that machinery generates transactions silently on startup.
- `SubscriptionRenewalSnapshotStore` is a **derived scheduling mirror** in UserDefaults feeding the notification coordinator — `Subscription` is the source of truth. Any mutation of `Subscription` must call `refresh(from:)` in the same transaction boundary (all `SubscriptionService` writes, backup import, and the AppRootView startup pass do this). A second mutation path that forgets the refresh will leave renewal notifications silently stale.
- Subscription amounts are privacy-masked like other sensitive figures; renewal notification bodies carry the subscription name but **never the amount** — lock-screen notifications bypass the in-app privacy lock.
...
- Cash-flow forecasting separates certainty instead of drawing one falsely precise line: recurring/installment events land on exact dates, while ordinary daily-budget spending uses completed recorded weeks at P20/P50/P80. Entirely blank weeks are missing samples, but a week with any record and zero qualifying spending is a real zero; the current partial week and recurring-generated transactions are excluded. Fewer than 4 observed weeks show one preliminary trend, 4–7 show a limited interval, and 8+ show the full historical range. This is not a probability or confidence interval. While privacy is locked, do not render the real chart at all—axes and shapes leak scale even when labels are masked.
...
- The ledger opens on `.payCycle`, not all history. On the configured payday a new cycle begins, so the page can legitimately show zero visible rows while hundreds of historical transactions still exist. Never infer an empty store, reseed, or offer destructive recovery from the filtered result. Empty-state copy and actions must distinguish “this range has no records” from “the ledger has no records” and provide a direct path to `.all` when history exists.
